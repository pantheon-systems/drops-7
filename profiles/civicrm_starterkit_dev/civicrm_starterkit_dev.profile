 ...
 */
function civicrm_starterkit_dev_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function civicrm_starterkit_dev_install_tasks_alter(&$tasks, $install_state) {

  // substitute our own finished step
  $tasks['install_finished'] = array(
    'display_name' => st('Finished Installation'),
    'display' => 1,
    'function' => 'civicrm_starterkit_dev_install_finished',
  );
    
} 

function civicrm_starterkit_dev_install_finished(&$install_state) {
 ...
 */
function civicrm_starterkit_dev_update_projects_alter(&$projects) {
  // Enable update status for the profile.
  $modules = system_rebuild_module_data();
  // The module object is shared in the request, so we need to clone it here.
  $profile = clone $modules['civicrm_starterkit_dev'];
  $profile->info['hidden'] = FALSE;
  _update_process_info_list($projects, array('civicrm_starterkit_dev' => $profile), 'module', TRUE);
}
 ...
 */
function civicrm_starterkit_dev_update_status_alter(&$projects) {
  $bad_statuses = array(
    UPDATE_NOT_SECURE,
    UPDATE_REVOKED,
  ...
  $make_filepath = drupal_get_path('module', 'civicrm_starterkit_dev') . '/drupal-org.make';
