...
<?php
  // prepare image
  if (!empty($content['field_slider_link']['#items'])) {
    $link = trim(render($content['field_slider_link']));
    if (!empty($content['field_slider_image'])) {
      $image = l(render($content['field_slider_image']), $link, array('html' => TRUE));
    }
    if (!empty($content['field_slider_caption'])) {
      $text = $content['field_slider_caption'][0]['#markup'] . '&nbsp;<i class="fa fa-external-link-square"></i>';
      $caption = l($text, $link, array('html' => TRUE));
    }
  } else {
    if (!empty($content['field_slider_image'])) {
